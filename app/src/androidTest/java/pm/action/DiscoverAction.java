...
public class DiscoverAction extends VP2 {
    ...
    public static int getZanNumber() {
        ...
        waitTime(5);
        List<UiObject2> linearLayouts = swip.findObjects(By.clazz(LinearLayout.class));
        logger.info(linearLayouts.size() + "");
        int like = 0;
        for (UiObject2 linearLayout : linearLayouts) {
            List<UiObject2> textViews = linearLayout.findObjects(By.depth(1).clazz(TextView.class));
            if (textViews.size() == 3) {
    ...
    public static String checkMiniProfileNumFollowerAddOneAfterFollow() throws UiObjectNotFoundException {
        ...
        String sioEye_id = getObject2ById(DiscoverPage.ID_PROFILE_MINI_DEVICES).findObject(By.clazz(TextView.class)).getText();
        //id for user
    ...
    public static int getPersonNumber() {
        ...
        UiObject2 swipe_target = getObject2ById(DiscoverPage.ID_Swipe_target);
        waitTime(5);
        List<UiObject2> linearLayouts = swipe_target.findObjects(By.clazz(LinearLayout.class));
        logger.info(linearLayouts.size() + "");
        int person = 0;
        boolean find=false;
        ...
        if (find==false){
            linearLayouts = swipe_target.findObjects(By.clazz(LinearLayout.class));
            for (UiObject2 linearLayout : linearLayouts) {
                textViews = linearLayout.findObjects(By.depth(1).clazz(TextView.class));
                if (textViews.size() == 2) {
