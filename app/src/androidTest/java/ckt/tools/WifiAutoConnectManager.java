...
public class WifiAutoConnectManager extends VP {
    ...
    private WifiConfiguration createWifiInfo(String SSID, String Password, WifiCipherType Type) {
        ...
        if (Type == WifiCipherType.WIFICIPHER_WPA) {
            ...
            Logger.getLogger(TAG).info(config.toString());
        }
        return config;
    }
  ...
  　　4 wifiManager..WIFI_STATE_ENABLING  (2)*/
    ...
    class ConnectRunnable implements Runnable {
        ...
        public void run() {
            ...
            //
            if (wifiConfig == null) {
                Logger.getLogger(TAG).info("wifiConfig is null!");
                return;
            }
            int netID = wifiManager.addNetwork(wifiConfig);
            boolean enabled = wifiManager.enableNetwork(netID, true);
            Logger.getLogger(TAG).info("enableNetwork status enable=" + enabled);
            boolean connected = wifiManager.reconnect();
            Logger.getLogger(TAG).info("enableNetwork connected=" + connected);
        }
    }
